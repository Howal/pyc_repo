...
class posenet_v1_resnet101_dilated_c5_sc(Symbol):
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        for _stage in range(stage_num):
            ...
            data = mx.sym.Deconvolution(data=data, num_filter=256, kernel=(4, 4), stride=(2, 2),
                                        ...
                                        name=prefix + '_deconv')
            # data = resnet_v1.bn_layer_wrapper(use_bn_type=use_bn_type, data=data, name=prefix+'_bn1', bn_mom=0.9)
    ...
    def init_weight_simple_baseline(self, cfg, arg_params, aux_params):
        ...
        for _stage in range(2):
            ...
            if cfg.network.use_bn_type:
                arg_params[prefix + '_bn1_gamma'] = mx.random.uniform(0, 1, shape=self.arg_shape_dict[prefix + '_bn1_gamma'])
                arg_params[prefix + '_bn1_beta'] = mx.nd.zeros(shape=self.arg_shape_dict[prefix + '_bn1_beta'])
                if cfg.network.use_bn_type != 'gn':
                    aux_params[prefix + '_bn1_moving_mean'] = mx.nd.zeros(
                        shape=self.aux_shape_dict[prefix + '_bn1_moving_mean'])
                    aux_params[prefix + '_bn1_moving_var'] = mx.nd.ones(shape=self.aux_shape_dict[prefix + '_bn1_moving_var'])
                arg_params[prefix + '_bn2_gamma'] = mx.random.uniform(0, 1, shape=self.arg_shape_dict[prefix + '_bn2_gamma'])
                arg_params[prefix + '_bn2_beta'] = mx.nd.zeros(shape=self.arg_shape_dict[prefix + '_bn2_beta'])
                if cfg.network.use_bn_type != 'gn':
                    aux_params[prefix + '_bn2_moving_mean'] = mx.nd.zeros(
                        shape=self.aux_shape_dict[prefix + '_bn2_moving_mean'])
                    aux_params[prefix + '_bn2_moving_var'] = mx.nd.ones(shape=self.aux_shape_dict[prefix + '_bn2_moving_var'])
