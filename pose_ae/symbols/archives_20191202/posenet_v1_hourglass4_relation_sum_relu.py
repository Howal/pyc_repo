...
class posenet_v1_hourglass4_relation_sum_relu(Symbol):
    ...
    def relation_module(self, key_data, query_data, affinity_dim, value_dim, output_dim, num_part, top_k,
        ...
        key_embd_reshape = key_embd.reshape(shape=(0, 0, -1))
        key_embd_reshape = mx.sym.transpose(key_embd_reshape, axes=(0, 2, 1))

        # value_embd_reshape: [N, num_part2 * K, Val_Dim]
        value_embd_reshape = value_embd.reshape(shape=(-1, value_dim, num_part * top_k),
                                                name=prefix + "_value_embd_reshape")  # [N * num_part1, Val_Dim, num_part2 * K]
        ...
        aff_mat = mx.sym.batch_dot(lhs=query_embd_reshape, rhs=key_embd_reshape, transpose_a=False,
                                   transpose_b=True, name=prefix + '_aff_mat_batch_dot') / (affinity_dim ** 2)
        # aff_mat = [N, H*W, num_part2, K]
        ...
        relation_feat = mx.sym.batch_dot(lhs=aff_mat_norm, rhs=value_embd_reshape, transpose_a=False,
                                         transpose_b=False)
        # relation_feat: [N, Val_dim, H*W]
        relation_feat = mx.sym.swapaxes(relation_feat, 1, 2)

        # relation_feat: [N, Val_dim, H*W, 1]
