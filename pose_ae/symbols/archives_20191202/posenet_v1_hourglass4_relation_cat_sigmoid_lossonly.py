...
class posenet_v1_hourglass4_relation_cat_sigmoid_lossonly(Symbol):
    ...
    def relation_module(self, key_data, query_data, affinity_dim, value_dim, output_dim, num_part, top_k,
        ...
        aff_mat_norm = mx.sym.broadcast_div(aff_mat_sigmoid, aff_mat_sigmoid.sum(axis=-1, keepdims=True))
    ...
    def get_stacked_hourglass(self, data, keypoint_location, keypoint_visible, num_stack=4, in_dim=256, out_dim=68, increase_dim=128,
        ...
        for i in range(cfg.pose.head_num):
            ...
            cross_entropy_loss = -aff_prob_gt*mx.sym.log(masked_fg_aff + 1e-5) + (1-aff_prob_gt) * mx.sym.log(1 - masked_fg_aff + 1e-5)
            ...
            #feature = mx.sym.concat(feature, dim=1)
        ...
        if is_train:
            ...
            if cfg.TRAIN.GPU_METRIC:
                ...
                if cfg.pose.aff_supervision:
                    for i in range(cfg.pose.head_num):
                        output_list.extend(get_aff_l2_loss(aff_losses[i]))
    ...
    def init_weight_non_local(self, cfg, arg_params, aux_params):
        prefix_name = "head_{}"
        weight_names = ['key_embed', 'query_embed', 'after_concat_1x1', 'det', 'association', 'sensitve_conv']
        bias_names = ['after_concat_1x1', 'det', 'association','sensitve_conv']
        for i in range(cfg.pose.head_num):
