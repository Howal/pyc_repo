...
class posenet_v1_hourglass4_relation_cat_clsagno_wo_embed(Symbol):
    ...
    def relation_module(self, key_data, query_data, affinity_dim, value_dim, output_dim, num_part, top_k,
        ...
        query_embd_reshape = query_data.reshape(shape=(0, 0, -1))  # [N, Aff_dim, H*W]
        query_embd_reshape = mx.sym.transpose(query_embd_reshape, axes=(0, 2, 1))  # [N, H*W, Aff_dim]

        # key_embd_reshape: [N, num_part2 * K, Aff_Dim]
        key_embd_reshape = mx.sym.transpose(key_data, axes=(0, 2, 3, 1))  # [N, num_part2, K, Aff_Dim]
    ...
    def get_stacked_hourglass(self, data, num_stack=4, in_dim=256, out_dim=68, increase_dim=128,
        ...
        for i in range(cfg.pose.head_num):
            ...
            select_part_indices = mx.sym.Custom(op_type="select_part", kernel=cfg.pose.nms, top_k=top_k, det_score=d_pred, cls_agnostic=True,
                                                name=prefix_name + "_select_part")
            # data_reshape: [N, num_parts, Dim, H, W]
            data_reshape = mx.sym.reshape(feature, shape=(0, -4, 1, -1, 0, 0))
            ...
            # part_feat = mx.symbol.Custom(op_type='monitor', data=part_feat, nickname="part_feat")
            ...
            relation_feat = self.relation_module(key_data=part_feat, query_data=feature,
                                                 ...
                                                 output_dim=in_dim,
                                                 num_part=17, top_k=top_k, prefix=prefix_name)
    ...
    def init_weight_non_local(self, cfg, arg_params, aux_params):
        prefix_name = "head_{}"
        weight_names = ['val_embed', 'fusion', 'after_concat_1x1', 'det', 'association']
