...
class posenet_v1_hourglass4_relationhg_cat_sigmoid(Symbol):
    ...
    def relation_module(self, key_data, query_data, affinity_dim, value_dim, output_dim, num_part, top_k,
        ...
        return relation_feat, aff_mat_sigmoid
    ...
    def get_stacked_hourglass(self, data, keypoint_location, keypoint_visible, num_stack=4, in_dim=256, out_dim=68, increase_dim=128,
        ...
        input_feat = data_preds + data_feats
        for i in range(cfg.pose.head_num):
            prefix_name="head_{}".format(i)

            top_k = cfg.pose.top_k

            pose_sensitive_feature = mx.sym.Convolution(data=input_feat, kernel=(1, 1), stride=(1, 1),
            ...
            relation_feat, aff_mat_sigmoid = self.relation_module(key_data=part_feat, query_data=input_feat,
            ...
            # masked_fg_aff, aff_prob_gt = mx.symbol.Custom(op_type='monitor_multivar', data_0=masked_fg_aff, data_1=aff_prob_gt, n=2, nickname="masked_fg_aff")
            ...
            #aff_loss = mx.symbol.Custom(op_type='monitor_multivar', data_0=aff_loss, data_1=aff_gt, n=2, nickname="aff_loss")

            input_feat = mx.sym.concat(input_feat, relation_feat, dim=1)
            input_feat = mx.sym.Convolution(data=input_feat, kernel=(1,1), stride=(1,1), num_filter=256, name=prefix_name+"_after_concat_1x1")
            input_feat = mx.sym.Activation(data=input_feat, act_type='relu')

            d_pred =  mx.sym.Convolution(data=input_feat, kernel=(1,1), stride=(1,1), num_filter=num_parts, name=prefix_name+"_det")
            a_pred = mx.sym.Convolution(data=input_feat, kernel=(1, 1), stride=(1,1), num_filter=num_parts, name=prefix_name + "_association")

            #input_feat_trans = mx.sym.Convolution(data=input_feat, kernel=(1,1), stride=(1,1), num_filter=256, name=prefix_name+"_feat_trans")
            d_pred_trans = mx.sym.Convolution(data=input_feat, kernel=(1,1), stride=(1,1), num_filter=256, name=prefix_name+"_det_trans")
            a_pred_trans = mx.sym.Convolution(data=input_feat, kernel=(1,1), stride=(1,1), num_filter=256, name=prefix_name+"_association_trans")

            input_feat = input_feat + d_pred_trans + a_pred_trans
    ...
    def init_weight_non_local(self, cfg, arg_params, aux_params):
        prefix_name = "head_{}"
        weight_names = ['key_embed', 'val_embed', 'query_embed', 'fusion', 'after_concat_1x1', 'det',
                        'association', 'sensitve_conv']
        ...
        weight_names =  ['det_trans', 'association_trans']
        bias_names = ['det_trans', 'association_trans']
        for i in range(cfg.pose.head_num-1):
            for weight_name in weight_names:
                ...
                if cfg.pose.param_init == 'normal':
                    arg_params[(weight_name + '_weight').format(i)] = mx.random.normal(0, 0.01,
                                                                                       shape=self.arg_shape_dict[
                                                                                           (
                                                                                           weight_name + '_weight').format(
                                                                                               i)])
                else:
                    arg_params[(weight_name + '_weight').format(i)] = mx.random.uniform(-bound, bound,
                                                                                        shape=weight_shape)
            for bias_name in bias_names:
                bias_name = prefix_name + "_" + bias_name
                if cfg.pose.param_init == 'normal':
                    arg_params[(bias_name + '_bias').format(i)] = mx.nd.zeros(
                        shape=self.arg_shape_dict[(bias_name + '_bias').format(i)])
                else:
                    arg_params[(bias_name + '_bias').format(i)] = mx.random.uniform(-bound, bound, self.arg_shape_dict[
                        (bias_name + '_bias').format(i)])
