...
class posenet_v1_hourglass4_pnn_align(Symbol):
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            max_proposals = cfg.pose.fg_proposals + cfg.pose.bg_proposals
        else:
            max_proposals = cfg.pose.max_proposals
        ...
        if is_train:
            ...
            if cfg.pose.use_offset:
                ...
                offset_var = mx.sym.Variable(name='offset_var')
                offset_label = mx.sym.Variable(name='offset_label')
        ...
        part_cls_reshape = mx.sym.Reshape(part_cls, shape=(-1, max_proposals, num_parts, 2))
        ...
        if cfg.pose.use_offset:
            ...
            # pred_offset = mx.sym.Custom(op_type='monitor', data=pred_offset, nickname='pred_offset')
            pred_offset_reshape = mx.sym.reshape(pred_offset, shape=(-1, max_proposals, num_parts, 2), name="pred_offset_reshape")
        ...
        if is_train:
            ...
            if cfg.pose.use_offset:
                offset_weight = offset_label == 1
                offset_var_reshape = offset_var.reshape(shape=(1, 1, num_parts, 1))
                offset_var_reshape_sqrt = mx.sym.sqrt(offset_var_reshape)
                offset_diff = pred_offset_reshape - gt_offset
                weighted_offset_diff = mx.sym.broadcast_div(offset_diff, offset_var_reshape_sqrt)

                offset_loss = offset_weight * mx.sym.smooth_l1(scalar=1.0, data=weighted_offset_diff).sum(axis=-1)
                offset_loss = offset_loss.sum() / mx.sym.maximum(1.0, offset_weight.sum())
    ...
    def init_weight_pnn(self, cfg, arg_params, aux_params):
        ...
        if cfg.pose.use_offset:
            ...
            arg_params['pred_offset_weight'] = mx.random.normal(shape=self.arg_shape_dict['pred_offset_weight'])
            arg_params['pred_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['pred_offset_bias'])
