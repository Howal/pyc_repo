...
class posenet_v1_hourglass4_bk(Symbol):
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            for i in range(num_stack):
                ...
                for j in range(max_persons):
                    for k in range(j + 1, max_persons):
                        ...
                        # tmp_a_loss_outside = mx.sym.Custom(data=tmp_a_loss_outside, op_type='monitor',
                        #                                      nickname='tmp_a_loss_outside_{0}_{1}_{2}'.format(i, j, k))

                        # keypoints_valid_list2[tmp_ids] = mx.sym.Custom(data=keypoints_valid_list2[tmp_ids], op_type='monitor',
                        #                                          nickname='keypoints_valid_list2[tmp_ids]_{0}'.format(i))
                ...
                # tmp_a_loss_outside_scale = mx.sym.Custom(data=tmp_a_loss_outside_scale, op_type='monitor',
                #                                    nickname='tmp_a_loss_outside_scale_{0}'.format(i))
                # tmp_a_loss_outside = mx.sym.Custom(data=tmp_a_loss_outside, op_type='monitor',
                #                                    nickname='tmp_a_loss_outside_{0}'.format(i))
                tmp_a_loss_outside_mean = mx.symbol.sum(tmp_a_loss_outside * tmp_a_loss_outside_scale, axis=1)  # shape, [N]
                # tmp_a_loss_outside_mean = mx.sym.Custom(data=tmp_a_loss_outside_mean, op_type='monitor',
                #                                    nickname='tmp_a_loss_outside_mean_{0}'.format(i))
            ...
            # a_loss_outside = mx.sym.Custom(data=a_loss_outside, op_type='monitor',
            #                                         nickname='a_loss_outside');
